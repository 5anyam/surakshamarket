...
export default function Header() {
  ...
  return (
    <>
      ...
      <header className={`sticky ${headerTop} z-40 bg-white border-b border-gray-200 shadow-sm transition-all duration-300`}>
          ...
          <div className="flex items-center justify-between h-14 md:h-16">
            ...
            <div className="flex items-center">
              {isMobile ? (
                <div className="flex items-center gap-1">
                  <button onClick={() => setMobileMenuOpen(true)} className="p-2 text-gray-500 hover:text-gray-900">
                    <HiOutlineMenuAlt3 className="text-2xl" />
                  </button>
                  <button onClick={() => setShowMobileSearch(true)} className="p-2 text-gray-500 hover:text-gray-900">
                ...
                <button
                  ...
                  className="p-2 text-gray-500 hover:text-[#2563eb] transition-colors"
                >
                  <FiSearch className="w-5 h-5" />
                </button>
              )}
            ...
            <Link href="/" className="absolute left-1/2 -translate-x-1/2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-sora font-extrabold text-lg md:text-xl tracking-tight text-gray-900 whitespace-nowrap">
              ...
              {!isMobile && (
                <div className="relative" ref={userMenuRef}>
                  {isAuthenticated ? (
                    ...
                    <Link
                      href="/login"
                      className="text-[11px] font-medium uppercase tracking-[0.15em] text-gray-500 hover:text-gray-900 transition-colors"
                    >
                      Sign In
                    </Link>
                  )}
                  {showUserMenu && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white border border-gray-200 shadow-xl py-2 z-50">
                      ...
                      <Link href="/account" className="block px-5 py-2 text-xs text-gray-600 hover:text-gray-900 hover:bg-gray-50">My Account</Link>
                      <button onClick={handleLogout} className="w-full text-left px-5 py-2 text-xs text-red-500 hover:bg-gray-50 border-t border-gray-100 mt-1">
                        Logout
                      </button>
                    </div>
                  )}
          ...
          <div className="bg-gray-50 border-t border-gray-200 px-4 py-4">
            <form onSubmit={handleSearch} className="max-w-2xl mx-auto flex items-center">
              <input
                ...
                className="w-full bg-transparent border-b border-gray-300 py-2 text-sm text-gray-900 focus:outline-none placeholder-gray-400 focus:border-[#2563eb] transition-colors"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                autoFocus
              />
              <button type="submit" className="ml-4 text-xs font-semibold uppercase tracking-widest text-[#2563eb] hover:text-[#1d4ed8] transition-colors">
                Search
              </button>
              <button type="button" onClick={() => setShowDesktopSearch(false)} className="ml-6 text-gray-400 hover:text-gray-700 transition-colors">
              ...
              {navItems.map((item) => (
                ...
                >
                  ...
                  {item.submenu && activeSubmenu === item.name && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 bg-white border border-gray-200 min-w-[220px] shadow-xl py-3 z-50">
                      {item.submenu.map((sub) => (
                        <Link
                          key={sub.name}
                          href={sub.to}
                          className="block px-5 py-2.5 text-[11px] tracking-wider text-gray-500 hover:text-gray-900 hover:bg-gray-50 transition-colors"
                        >
                          {sub.name}
                        </Link>
                      ))}
      ...
      {mobileMenuOpen && (
        <>
          ...
          />
          <div className={`fixed ${mobileDrawerTop} left-0 h-full w-full max-w-[300px] bg-white z-50 overflow-y-auto border-r border-gray-200 shadow-2xl`}>

            <div className="p-5 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-[#2563eb]" />
                <span className="font-sora font-bold text-base text-gray-900">
                  Suraksha<span className="text-[#2563eb]">Market</span>
                </span>
              </div>
              <button onClick={() => setMobileMenuOpen(false)} className="p-1.5 border border-gray-200 hover:border-[#2563eb] transition-colors">
            ...
            <nav className="p-5">
              {navItems.map((item) => (
                <div key={item.name} className="mb-1">
                  <div className="flex items-center justify-between border-b border-gray-100 py-3.5">
                    <Link
                      href={item.to}
                      className={`text-sm font-medium tracking-widest ${item.name === 'DEALS' ? 'text-[#2563eb]' : 'text-gray-700 hover:text-gray-900'}`}
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      {item.name}
                    </Link>
                    {item.submenu && (
                      <button onClick={() => setMobileActiveSubmenu(mobileActiveSubmenu === item.name ? null : item.name)}>
                        <BiChevronDown className={`text-xl text-gray-400 transition-transform ${mobileActiveSubmenu === item.name ? 'rotate-180' : ''}`} />
                      </button>
                    )}
                  </div>
                  {item.submenu && mobileActiveSubmenu === item.name && (
                    <div className="bg-gray-50 px-4 py-2 flex flex-col gap-2.5">
                      {item.submenu.map((sub) => (
                        <Link
                          key={sub.name}
                          href={sub.to}
              ...
              <div className="mt-10 space-y-5 border-t border-gray-100 pt-6">
                <Link href="/account" className="flex items-center gap-3 text-xs tracking-widest text-gray-500 uppercase hover:text-gray-900">
                  <UserCircle2 className="w-4 h-4 stroke-[1.5]" /> My Account
                </Link>
                <a href="tel:+919911636888" className="flex items-center gap-3 text-xs tracking-widest text-gray-500 uppercase hover:text-gray-900">
        ...
        <div className="fixed inset-0 bg-white z-[60] p-6">
          ...
          <form onSubmit={handleSearch}>
            <input
              ...
              type="text"
              placeholder="Search security products..."
              className="w-full text-lg border-b border-gray-300 bg-transparent pb-4 focus:outline-none placeholder-gray-400 text-gray-900 focus:border-[#2563eb] transition-colors"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </form>
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_SEARCH_CHIPS.map((chip) => (
                <button
                  ...
                  className="px-4 py-2 border border-gray-200 bg-gray-50 text-xs font-medium text-gray-600 hover:border-[#2563eb] hover:text-[#2563eb] transition-colors"
                >
                  {chip}
                </button>
              ))}
