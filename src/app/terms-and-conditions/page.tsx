...
const termsSections: TermsSection[] = [
    ...
    content: (
      <div className="space-y-3">
        <p className="text-sm text-gray-600 leading-relaxed">
          ...
          <span className="font-semibold text-[#2563eb]">7 days of delivery</span> with valid proof or documented evidence.
        </p>
        <ul className="space-y-2">
          {[
            ...
            <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
              <span className="w-1.5 h-1.5 bg-[#2563eb] rounded-full flex-shrink-0 mt-2" />
              {item}
            </li>
          ))}
        </ul>
  ...
  },
  {
    ...
    content: (
      <p className="text-sm text-gray-600 leading-relaxed">
        All creative assets, including brand logos, product visuals, digital content, and layout designs, are the intellectual property of Suraksha Market or their rightful owners. Any unauthorized use, reproduction, or distribution constitutes a legal violation and may invite immediate legal proceedings.
      </p>
    ),
  },
  {
...
const relatedPolicies: PolicyLink[] = [
  ...
  },
  {
    ...
    description: 'Learn when and how you can cancel an order on Suraksha Market.',
...
export default function TermsOfService() {
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      {/* ── HERO ── */}
      <section className="bg-[#1e40af] py-14 px-4 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <p className="text-blue-200 text-sm leading-relaxed max-w-2xl">
            Welcome to Suraksha Market. By using our platform or making a purchase, you agree to the terms and conditions that govern your use of our services.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto px-4 py-12 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <p className="text-sm text-gray-600 leading-relaxed">
            ...
            <a href="https://www.surakhshamarket.in" className="text-[#2563eb] font-medium hover:underline">
              www.surakhshamarket.in
            </a>{' '}
        ...
        {termsSections.map((section, index) => (
          <div
            key={index}
            className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:border-[#2563eb]/20 hover:shadow-md transition-all duration-300"
          >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { icon: Mail, label: 'Email', value: 'support@surakhshamarket.in', href: 'mailto:support@surakhshamarket.in' },
              { icon: Phone, label: 'Phone / WhatsApp', value: '+91 9911636888', href: 'tel:+919911636888' },
            ].map((item, i) => (
              <a
                key={i}
                href={item.href}
                className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-[#2563eb]/30 hover:bg-orange-50/30 transition-all duration-200 group"
              >
                <div className="w-9 h-9 bg-orange-50 rounded-lg flex items-center justify-center group-hover:bg-[#2563eb] transition-colors">
                  <item.icon className="w-4 h-4 text-[#2563eb] group-hover:text-white transition-colors" />
                </div>
                <div>
          ...
          <div className="mt-4 flex items-start gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
            ...
            <div>
              ...
              <address className="text-sm text-gray-700 not-italic leading-relaxed">
                Suraksha Market, Sector 15, Rohini<br />
        ...
        {/* Closing Statement */}
        <div className="bg-[#1e40af] rounded-2xl p-7 text-white text-center relative overflow-hidden">
          ...
          <p className="relative z-10 text-sm text-blue-200 leading-relaxed max-w-2xl mx-auto">
            By using the Suraksha Market website, you acknowledge that you have{' '}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a
              href="mailto:support@surakhshamarket.in"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#2563eb] hover:bg-[#1d4ed8] text-white rounded-xl text-sm font-bold uppercase tracking-wide transition-all shadow-md"
            >
            ...
            <Link
              href="/"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white border-2 border-[#1e40af] text-[#1e40af] hover:bg-[#1e40af] hover:text-white rounded-xl text-sm font-bold uppercase tracking-wide transition-all"
            >
              Back to Home
            </Link>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {relatedPolicies.map((policy, index) => (
              <Link
                key={index}
                href={policy.link}
                className="group bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:border-[#2563eb]/20 hover:shadow-md transition-all duration-300"
              >
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-orange-50 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-[#2563eb] transition-colors duration-300">
                    <policy.icon className="w-5 h-5 text-[#2563eb] group-hover:text-white transition-colors duration-300" />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-sm font-bold text-gray-900 mb-1 group-hover:text-[#2563eb] transition-colors">
                      {policy.title}
                    </h3>
        ...
        <p className="text-xs text-gray-400 text-center pb-4">
          © {new Date().getFullYear()} Suraksha Market. All rights reserved. &nbsp;·&nbsp;{' '}
          <Link href="/privacy-policy" className="text-[#2563eb] hover:underline">Privacy Policy</Link>
          {' '}&nbsp;·&nbsp;{' '}
          <Link href="/disclaimer" className="text-[#2563eb] hover:underline">Disclaimer</Link>
        </p>
      </div>
    </main>
  );
