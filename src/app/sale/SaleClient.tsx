      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <span className="text-[#2563eb] font-semibold">Sale</span>
          </div>
        </div>
      </div>

      {/* ── HERO BANNER ── */}
      <div className="relative bg-[#1e40af] overflow-hidden">
        {/* Background pattern */}
        <div className="absolute inset-0 opacity-5"
          style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #2563eb 0%, transparent 50%), radial-gradient(circle at 80% 20%, #2563eb 0%, transparent 40%)' }}
        />
        <div className="relative max-w-7xl mx-auto px-4 py-12 md:py-16 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <Flame className="w-5 h-5 text-[#2563eb] animate-pulse" />
              <span className="text-[11px] font-bold uppercase tracking-[0.3em] text-[#2563eb]">
                Limited Time Deals
              </span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-white mb-2 leading-tight">
              SALE <span className="text-[#2563eb]">FEST</span>
            </h1>
          ...
          <div className="flex items-center gap-2 overflow-x-auto py-3 scrollbar-hide">
            <button
              ...
              className={`flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${
                activeCategory === 'all'
                  ? 'bg-[#2563eb] text-white shadow-md shadow-orange-200'
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="lg:hidden flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:border-[#2563eb]/50 hover:text-[#2563eb] transition-colors bg-white"
            >
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:border-[#2563eb]/50 hover:text-[#2563eb] transition-colors bg-white"
              >
                  ...
                  {(Object.keys(SORT_LABELS) as SortOption[]).map((key) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm flex items-center justify-between transition-colors
                        ${sortBy === key ? 'text-[#2563eb] bg-orange-50 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}
                    >
                      {SORT_LABELS[key]}
          ...
          <aside className="hidden lg:block w-60 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm sticky top-36">
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 mb-5 flex items-center gap-2">
                <Zap className="w-3.5 h-3.5 text-[#2563eb]" /> Filters
              </h3>
              ...
              <div className="pt-5 border-t border-gray-100">
                ...
                <p className="text-lg font-bold text-[#2563eb] mb-3">
              ...
              <button
                ...
                className="mt-5 text-xs text-[#2563eb] hover:underline font-medium"
              >
                Clear All Filters
              </button>
            </div>
          ...
          <div className="flex-1 min-w-0">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <button
                  onClick={() => { setActiveCategory('all'); setPriceMax(null) }}
                  className="px-6 py-3 bg-[#2563eb] text-white text-sm font-semibold rounded-xl hover:bg-[#1d4ed8] transition-colors shadow-md"
                >
                  Clear Filters
                </button>
              </div>
                ...
                {filtered.map((product) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-[#2563eb]/30 hover:shadow-lg transition-all duration-300"
                    >
                      {/* Image */}
                      <div className="relative aspect-square overflow-hidden bg-gray-50">
                        ...
                        <div className="absolute top-2 left-2">
                          <span className="bg-[#2563eb] text-white text-[11px] font-black px-2.5 py-1 rounded-full flex items-center gap-1 shadow-md">
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 p-2 bg-white rounded-full shadow-md border border-gray-100 hover:border-[#2563eb]/30 transition-all opacity-0 group-hover:opacity-100"
                          aria-label="Wishlist"
                        >
                          <Heart className={`w-3.5 h-3.5 transition-colors ${isWishlisted ? 'fill-[#2563eb] text-[#2563eb]' : 'text-gray-400'}`} />
                        </button>

                        {/* Quick Add */}
                        <button
                          onClick={(e) => handleAddToCart(product, e)}
                          className="absolute bottom-2 left-2 right-2 bg-[#2563eb] text-white text-xs font-bold py-2 rounded-xl flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 shadow-md"
                        >
                          {isAdding
                      ...
                      <div className="p-3 md:p-4">
                        ...
                        <h3 className="text-sm font-semibold text-gray-900 leading-snug line-clamp-2 mb-2 group-hover:text-[#2563eb] transition-colors">
                          {product.name}
                        </h3>
                        <div className="flex items-center gap-0.5 mb-2">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} className="w-3 h-3 text-[#2563eb] fill-[#2563eb]" />
                          ))}
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl z-50 p-6 shadow-2xl lg:hidden max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                    activeCategory === 'all' ? 'bg-[#2563eb] text-white' : 'bg-gray-100 text-gray-600'
                  }`}
                >
                  All
                </button>
                {categories.map((cat) => (
                  <button
                    ...
                    onClick={() => setActiveCategory(cat.slug)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                      activeCategory === cat.slug ? 'bg-[#2563eb] text-white' : 'bg-gray-100 text-gray-600'
                    }`}
                  >
                    {cat.name}
                  </button>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-[#2563eb] rounded-xl text-sm font-semibold text-white shadow-md"
              >
                Apply
              </button>
            </div>
